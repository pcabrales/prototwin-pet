...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[96,96,96],
        ...
        ########### Setting up the Deviation-Predicting Branch (DPB): ##########################
        i_layer = len(depths) - 2  # penultimate encoder layer
        bottleneck_layer_size = np.array([crop_size[0] // patch_size[0] // 2 ** i_layer, crop_size[1] // patch_size[1] // 2 ** i_layer,
                    crop_size[2] // patch_size[2] // 2 ** i_layer])
        flattened_bottleneck_layer_size= bottleneck_layer_size[0] * bottleneck_layer_size[1] * bottleneck_layer_size[2] * embed_dim * 2 ** i_layer
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(flattened_bottleneck_layer_size, 1024)  # For head, penultimate encoder layer
    ...
    def forward(self, x, return_bottleneck=False):
        # Set return_bottleneck to True to return the predictions from the DPB
        ...
        if return_bottleneck:
            # Implementing the DPB
            dpb_out = self.flatten(skips[-2]) # penultimate decoder layer
            dpb_out = torch.nn.functional.leaky_relu(self.fc1(dpb_out))
            dpb_out = torch.nn.functional.leaky_relu(self.fc2(dpb_out))
            dpb_out = torch.nn.functional.leaky_relu(self.fc3(dpb_out))   
            dpb_out = self.fc4(dpb_out)
