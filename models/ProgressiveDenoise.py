...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, t: torch.Tensor):
        ...
        emb = math.log(100) / (half_dim - 1)  # 10_000 to 100 to reflect that we only have ten time steps
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, time_channels: int,
                 n_groups: int = 4, dropout: float = 0.1):   ### n_groups: int = 16
        """
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, n_channels: int, n_heads: int = 1, d_k: int = None, n_groups: int = 4):  ###n_groups: int = 16
        """
    ...
    def forward(self, x: torch.Tensor, t: Optional[torch.Tensor] = None):
        """
        * `x` has shape `[batch_size, in_channels, height, width, length]`
        * `t` has shape `[batch_size, time_channels]`
        """
        ...
        _ = t
        # Get shape
        batch_size, n_channels, height, width, length = x.shape
        ...
        res = res.permute(0, 2, 1).view(batch_size, n_channels, height, width, length)
...
class UNetDDPM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t = None):
        ...
        * `t` has shape `[batch_size]`
        ...
        if t is not None:
            if torch.any(t == -1):  # -1 is a placeholder value for not specifying a time, because the dataset can't handle None types
                t = None
            else:
