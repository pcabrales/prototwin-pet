...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(24*32*24*32, 1024)  # For head, second to last encoder layer(enc2)
    ...
    def forward(self, x_in, return_bottleneck=False):
        ...
        if return_bottleneck:
            # x = self.flatten(enc3)  # penultimate encoder layerhead-sobp-swinunetr-v5: like v2 but using tanh at the last layer and scaling for outputs between -5 and 5 (had to divide by 100 because it would blow up otherwise)
            x = self.flatten(enc2)  # bottleneck layer
            x = torch.nn.functional.leaky_relu(self.fc1(x))
            x = torch.nn.functional.leaky_relu(self.fc2(x))
            x = torch.nn.functional.leaky_relu(self.fc3(x))   
            x = self.fc4(x)
            return logits, x  ### returning the last decoder output to compare between noisy samples of the same beam, as they should be identical
        else:
            return logits
