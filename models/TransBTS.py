...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        assert all(img_dim[i] % patch_dim == 0 for i in range(3))
        ...
        self.num_patches = int(img_dim[0] * img_dim[1] * img_dim[2] // patch_dim ** 3)
        ...
        if positional_encoding_type == "learned":
            self.position_encoding = LearnedPositionalEncoding(
                self.patch_dim, self.embedding_dim, self.seq_length
            )
...
class BTS(TransformerBTS):
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        y = self.endconv(y2)      # (1, 1, 128, 128, 128)
        return y
...
def TransBTS(img_dim=64, _conv_repr=True, _pe_type="learned"):
    num_channels = 1
    patch_dim = 8
    assert isinstance(img_dim, (int, tuple))
    if isinstance(img_dim, int):
        img_dim = (img_dim, img_dim, img_dim)
    model = BTS(
        img_dim,
        patch_dim,
        num_channels,
...
#model = TransBTS(img_dim=64, _conv_repr=True, _pe_type="learned")