...
class SegResNet(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(64*16*12*16, 1024)  # First layer with 512 units
        # self.fc1 = nn.Linear(96*20*4*4, 1024)  # For prostate: # First layer with 512 units
    ...
    def forward(self, x: torch.Tensor, return_bottleneck=False) -> torch.Tensor:
        ...
        if return_bottleneck:
            x_dev = self.flatten(x_bottleneck)  # x_dev is the output of the DPB, the deviations(dev) in set-up
            x_dev = torch.nn.functional.leaky_relu(self.fc1(x_dev))
            x_dev = torch.nn.functional.leaky_relu(self.fc2(x_dev))
            x_dev = torch.nn.functional.leaky_relu(self.fc3(x_dev))
            x_dev = self.fc4(x_dev)       
            return x, x_dev  ### returning the last decoder output to compare between noisy samples of the same beam, as they should be identical
        else:
            return x
