...
class UNetV14(nn.Module):
    def __init__(self):
        ...
        # Encoder layers
        self.e1 = self.ConvBlock(1, 16)
        self.e2 = self.ConvBlock(16, 32)
        self.e3 = self.ConvBlock(32, 64)
        self.e4 = self.ConvBlock(64, 128)

        # Bottleneck layer
        self.m1 = self.ConvBlock(128, 256)

        # Decoder layers
        self.upconv4 = nn.ConvTranspose3d(256, 128, kernel_size=2, stride=2)  # Upscaling
        self.d4 = self.ConvBlock(256, 128)
        self.upconv3 = nn.ConvTranspose3d(128, 64, kernel_size=2, stride=2)
        self.d3 = self.ConvBlock(128, 64)
        self.upconv2 = nn.ConvTranspose3d(64, 32, kernel_size=2, stride=2)
        self.d2 = self.ConvBlock(64, 32)
        self.upconv1 = nn.ConvTranspose3d(32, 16, kernel_size=2, stride=2)
        self.d1 = self.ConvBlock(32, 16)
    ...
    def forward(self, x):
        ...
        x_pool1 = self.pool(x_e1)
        x_e2 = self.e2(x_pool1)
        
        x_pool2 = self.pool(x_e2)
        x_e3 = self.e3(x_pool2)
        
        x_pool3 = self.pool(x_e3)
        x_e4 = self.e4(x_pool3)
        
        x_pool4 = self.pool(x_e4)
        x_m = self.m1(x_pool4)

        x_upconv4 = self.upconv4(x_m)
        x_upconv4 = torch.cat([x_upconv4, x_e4], dim=1)  # Concatenating
        x_d4 = self.d4(x_upconv4)
        
        x_upconv3 = self.upconv3(x_d4)
        x_upconv3 = torch.cat([x_upconv3, x_e3], dim=1)
        x_d3 = self.d3(x_upconv3)
        
        x_upconv2 = self.upconv2(x_d3)
        x_upconv2 = torch.cat([x_upconv2, x_e2], dim=1)
        x_d2 = self.d2(x_upconv2)
        
        x_upconv1 = self.upconv1(x_d2)
        x_upconv1 = torch.cat([x_upconv1, x_e1], dim=1)
        x_d1 = self.d1(x_upconv1)

        x_out = self.output(x_d1)
...
class AE(nn.Module):
    def __init__(self):
        ...
        # Decoder layers
        self.upconv4 = nn.ConvTranspose3d(256, 256, kernel_size=2, stride=2)  # Upscaling
        self.d4 = self.ConvBlock(256, 128)
        ...
        self.d2 = self.ConvBlock(64, 32)
        self.upconv1 = nn.ConvTranspose3d(32, 32, kernel_size=2, stride=2)
        self.d1 = self.ConvBlock(32, 16)
...
class RED_CNN(nn.Module):
    def __init__(self, out_ch=20):
        super(RED_CNN, self).__init__()
        self.conv1 = nn.Conv3d(1, out_ch, kernel_size=5, stride=1, padding=0)
        self.conv2 = nn.Conv3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.conv3 = nn.Conv3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.conv4 = nn.Conv3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.conv5 = nn.Conv3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)

        self.tconv1 = nn.ConvTranspose3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.tconv2 = nn.ConvTranspose3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.tconv3 = nn.ConvTranspose3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.tconv4 = nn.ConvTranspose3d(out_ch, out_ch, kernel_size=5, stride=1, padding=0)
        self.tconv5 = nn.ConvTranspose3d(out_ch, 1, kernel_size=5, stride=1, padding=0)
