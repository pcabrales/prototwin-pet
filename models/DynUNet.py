...
class DynUNetSkipLayer(nn.Module):
    ...
    def __init__(self, index, downsample, upsample, next_layer, heads=None, super_head=None, bottleneck_heads=None):
        super().__init__()
    ...
    def forward(self, x, return_bottleneck=False, ch_size_bottleneck=None):
        downout = self.downsample(x)
        if type(self.next_layer) == DynUNetSkipLayer:
            nextout = self.next_layer(downout, return_bottleneck, ch_size_bottleneck)
        else:
            nextout = self.next_layer(downout)
        # Manage deviation values delta x, y, psi: because the unet has a structure where for each layer the upsample, downsample and next layer are called, to get the bottlenck we need  to move it from the bottleneck layer up to the top
        if return_bottleneck and nextout.shape[1] == ch_size_bottleneck and self.bottleneck_heads is not None and self.index > 0:   ### to use the bottleneck tensor to find deviation values
            self.bottleneck_heads[self.index - 1] = nextout
...
class DynUNet(nn.Module):
    ...
    def __init__(
        ...
        self.bottleneck_heads: List[torch.Tensor] = [None] * (len(self.upsamples) - 1)   ### to use the bottleneck tensor to find deviation values
        ...
        def create_skips(index, downsamples, upsamples, bottleneck, superheads=None):
            ...
            if superheads is None:
                ...
                return DynUNetSkipLayer(index, downsample=downsamples[0], upsample=upsamples[0], next_layer=next_layer, bottleneck_heads=self.bottleneck_heads)
            ...
            if super_head_flag:
                return DynUNetSkipLayer(
                    ...
                    bottleneck_heads=self.bottleneck_heads
                )
        ...
        # For the deviation output ###
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(512*4*6*4, 1024)  # For head, second to last encoder layer(enc2)
        # self.fc1 = nn.Linear(48*16*12*16, 1024)  # For head, last encoder layer (enc3)
        # self.fc1 = nn.Linear(96*20*4*4, 1024)  # For prostate
        self.fc2 = nn.Linear(1024, 256)  # Second layer with 256 units
        self.fc3 = nn.Linear(256, 128)  # Continue decreasing
        # self.fc4 = nn.Linear(128, 4)   # For prostate: Final layer for the 4 numbers 
        self.fc4 = nn.Linear(128, 3)   # For head: Final layer for the 3 numbers 
        self.tanh = nn.Tanh()
    ...
    def forward(self, x, return_bottleneck=False):
        if return_bottleneck:
            out = self.skip_layers(x, return_bottleneck=return_bottleneck, ch_size_bottleneck=self.filters[-1])  # extracting the bottleneck if self.filters[-1], if further back then -2 and so on
            bottleneck_head = self.bottleneck_heads[-1]
            x = self.flatten(bottleneck_head)  # bottleneck layer
            x = torch.nn.functional.leaky_relu(self.fc1(x))
            x = torch.nn.functional.leaky_relu(self.fc2(x))
            x = torch.nn.functional.leaky_relu(self.fc3(x))   
            dpb_values = self.fc4(x)
        else:
