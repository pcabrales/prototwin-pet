...
def generate_sensitivity(
    ...
    with gzip.open(os.path.join(mcgpu_location, "image_Trues.raw.gz"), "rb") as f:
        sensitivity = (
            np.frombuffer(f.read(), dtype=np.int32).reshape(img_shape, order="F").copy()
        )
        print(
            sensitivity.min(), sensitivity.max(), sensitivity.mean(), sensitivity.shape
        )  ###
    sensitivity[
        sensitivity == 0
    ] = 1  # to avoid dividing by zero, only happens at the edge of FOV
...
def convert_CT_to_mhd(
    mhd_file, dicom_dir=None, matRad_output=None, image_size=(272, 272, 176)
):
