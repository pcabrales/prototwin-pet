...
class DoseActivityDataset(Dataset):
    ...
    def __init__(
        ...
        self.joint_transform = (
            joint_transform  # Transforms applied to both input and output images
        )
        self.energy_beam_dict = (
            energy_beam_dict  # Dictionary including the energy for each beam in keV
        )
        self.file_names = os.listdir(input_dir)[
            :num_samples
        ]  # Only selecting as many files as given by num_samples
        ...
        if (training_set or test_set) and energy_beam_dict:
            ...
            sorted_energies = sorted(
                energy_counts.keys()
            )  # Sorts the energies of all beams in the dictionary
            energy_threshold = float(
                sorted_energies[-4]
            )  # Selecting the train/test energy threshold to the fourth most powerful energies (there are around 50 different beam energies)
            number_beams = len(self.file_names)
            for idx in range(number_beams):
                beam = self.file_names[idx][:4]
                beam_energy = energy_beam_dict[beam]
                if (
                    test_set and float(beam_energy) >= energy_threshold
                ):  # If generating the test set and if a beam has an energy above the threshold, select it
                    self.file_names_new.append(self.file_names[idx])
                elif (
                    training_set and float(beam_energy) <= energy_threshold
                ):  # If generating the training set and if a beam has an energy below the threshold, select it
                    self.file_names_new.append(self.file_names[idx])
    ...
    def __getitem__(self, idx):
        ...
        if self.joint_transform:
            input_volume, output_volume = self.joint_transform(
                input_volume, output_volume
            )
        if self.CT_flag:
            input_volume = torch.cat(
                (input_volume, torch.tensor(self.CT, dtype=torch.float32).unsqueeze(0))
            )  # Placing the CT as a second input channel
        if self.energy_beam_dict is not None:
            beam_number = self.file_names[idx][:4]
            beam_energy = self.energy_beam_dict.get(
                beam_number, 0.0
            )  # If the energy is not in the dictionary, set to 0
...
class SOBPDoseActivityDataset(Dataset):
    ...
    def __init__(
        ...
        self.deviations_dict = (
            deviations_dict  # Dictionary including the energy for each beam in keV
        )
        self.file_names = os.listdir(input_dir)[
            :num_samples
        ]  # Only selecting as many files as given by num_samples
    ...
    def __getitem__(self, idx):
        ...
        if self.deviations_dict is not None:
            deviations = torch.tensor(
                self.deviations_dict[self.file_names[idx][:-4]], dtype=torch.float32
            )
...
def dataset_statistics(
    image_dir, scaling="standard", num_samples=5, joint_transform=None
):
    dataset = DoseActivityDataset(
        input_dir=image_dir,
...
def shuffled_loaders(datasets, batch_size, shuffle=True, num_workers=1):
    ...
    # Shuffle indices
    indices = (
        torch.randperm(dataset_length).tolist()
        if shuffle
        else list(range(dataset_length))
    )
...
class GaussianBlurFloats:
    ...
    def __call__(self, img):
        ...
        if random.random() < self.p:
            ...
            self.sigma = (
                random.random() * self.sigma
            )  # A random number between 0 and the max sigma
        ...
        blurred_tensor = torch.tensor(
            blurred_array, dtype=img.dtype, device=img.device
        ).unsqueeze(0)
...
class Random3DCrop:  # Crop image to be multiple of 8
    def __init__(self, output_size):
        ...
        )  # Check if size is int (all images have the same dim), or tuple
    ...
    def __call__(self, img):
        ...
        return img[:, top : top + new_h, left : left + new_w, front : front + new_d]
...
class Resize3D:
    ...
    def __call__(self, img):
        img = img.unsqueeze(
            0
        )  # Unsqueeze because interpolate takes input tensors that have a batch and channel dimensions
...
class ResizeCropAndPad3D:
    ...
    def __call__(self, img):
        img = img.unsqueeze(
            1
        )  # Pad takes input tensors that have a batch and channel dimensions
        pad_right = (
            pad_left
        ) = pad_top = pad_bottom = pad_front = pad_back = 0  # By default, do not pad
        ...
        if (
            self.size[0] > img.shape[2]
        ):  # dim=2 of img corresponds to the first spatial size (dim=0 of self.size)
...
class GaussianBlob:
    ...
    def __call__(self, img):
        ...
        bool_img = (
            img > 0.1 * max_overall
        )  # Looking for the position just after the BP, since we will place the blur between the start and that position
        bp_position = torch.where(bool_img.any(dim=1).any(dim=1))[0][0].item()

        # Create a 3D Gaussian kernel
        ax = torch.arange(-self.size // 2 + 1.0, self.size // 2 + 1.0)
        xx, yy, zz = torch.meshgrid(ax, ax, ax)
        kernel = (
            0.5
            * max_overall
            * torch.exp(-(xx**2 + yy**2 + zz**2) / (2 * self.sigma**2))
        )
        ...
        center = (
            random.randint(bp_position, img.shape[0] - 1),
            idx_max_overall[1],
            idx_max_overall[2],
        )
        ...
        kernel = kernel[
            (z_start - center[0] + self.size // 2) : (
                z_end - center[0] + self.size // 2
            ),
            (y_start - center[1] + self.size // 2) : (
                y_end - center[1] + self.size // 2
            ),
            (x_start - center[2] + self.size // 2) : (
                x_end - center[2] + self.size // 2
            ),
        ]
...
class StreakingArtifacts:
    def __init__(
        self, num_streaks=8, n_points=3, streak_thickness=2, streak_intensity=0.5
    ):
    ...
    def __call__(self, img):
        ...
        for y in range(y_voxels):
            ...
            for i in range(self.num_streaks // 2):
                ...
                ):  # Increment can be adjusted for finer lines
                    end_x = int(center_x + mult * np.cos(angle))
                    # floor of value
                    end_z = int(center_z + mult * np.sin(angle))

                    for end_x_offset in range(
                        -self.streak_thickness, self.streak_thickness + 1
                    ):
                        for end_z_offset in range(
                            -self.streak_thickness // 4, self.streak_thickness // 4 + 1
                        ):
                            if (
                                end_x + end_x_offset < 0
                                or end_x + end_x_offset >= x_voxels
                                or end_z + end_z_offset < 0
                                or end_z + end_z_offset >= z_voxels
                            ):
                                break
                            # Apply the streak color (do not let it get over the image max)
                            slice_img[end_x + end_x_offset, end_z + end_z_offset] = min(
                                img_max,
                                self.streak_intensity * img_max
                                + slice_img[end_x + end_x_offset, end_z + end_z_offset],
                            )
        # Convert back to tensor
        streaky_tensor = torch.tensor(
            streaky_img, dtype=img.dtype, device=img.device
        ).unsqueeze(0)
...
def RE_loss(output, target, mean_output=0, std_output=1):  # Relative error loss
    ...
    loss = (
        abs_diff / max_intensity.view(-1, 1, 1, 1) * 100
    )  # Loss is a tensor in which each pixel contains the relative error
    return loss
...
def post_BP_loss(output, target, device="cpu", mean_output=0, std_output=1):
    ...
    max_along_depth, idx_max_along_depth = torch.max(
        target, dim=1
    )  # Max at each transversal point
    indices_keep = max_along_depth > 0.01 * max_global.unsqueeze(-1).unsqueeze(
        -1
    )  # Unsqueeze to match dimensions of the tensors. These are the indices of the transversal Bragg Peaks higher than 1% of the highest peak BP
    max_along_depth = max_along_depth[
        indices_keep
    ]  # Only keep the max, or bragg peaks, of transversal points with non-null dose
    ...
    mask_pre_BP = (
        depth[:, None] > idx_max_along_depth
    )  # mask to only consider the range after the bragg peak (indices smaller than the index at the BP)
    mask_post_BP = (
        ddp_data < 0.1 * max_along_depth
    )  # mask to only consider the range between the BP and the drop until 0.1 * BP
...
# Range deviation
def range_loss(
    output, target, range_val=0.9, device="cpu", mean_output=0, std_output=1
):
    ...
    indices_keep = max_along_depth > 0.05 * max_global.unsqueeze(-1).unsqueeze(
        -1
    )  # Unsqueeze to match dimensions of the tensors. These are the indices of the transversal Bragg Peaks higher than 1% of the highest peak BP
    max_along_depth = max_along_depth[
        indices_keep
    ]  # Only keep the max, or bragg peaks, of transversal points with non-null dose (above 0.1 of max)
    idx_max_along_depth = idx_max_along_depth[indices_keep]
    # target_permuted = torch.permute(target, (1, 0, 2, 3))
    # output_permuted = torch.permute(output, (1, 0, 2, 3))
    target_permuted = manual_permute(target, (1, 0, 2, 3))
    ...
    ddp_output_interp1d = interp1d(
        depth.numpy(), ddp_output_data.numpy(), axis=0, kind="cubic"
    )
    ...
    for col in ddp_depth_extended.T:
        ...
        if np.max(idcs_above_90_of_max) - np.min(idcs_above_90_of_max) > 20:
            ddp_depth_extended = np.delete(
                ddp_depth_extended, n_col, 1
    ...
    depth_at_range = depth_extended[
        torch.argmin(torch.abs(ddp_depth_extended - dose_at_range), dim=0)
    ]
    depth_at_range_output = depth_extended[
        torch.argmin(torch.abs(ddp_output_depth_extended - dose_at_range_output), dim=0)
    ]
...
def plot_range_histogram(range_list, save_plot_dir="images/hist_BP_deviation.jpg"):
    ...
    range_list = range_list[range_list > -5]
    plt.hist(
        range_list.flatten(), bins=bins, color="blue", alpha=0.5
...
def gamma_index(output, target, tolerance=0.03, beta=5, threshold=0.2):
    ...
    max_target = torch.amax(
        target, dim=(1, 2, 3)
    )  # Overall max for each image in the batch
    target = target / max_target.unsqueeze(-1).unsqueeze(-1).unsqueeze(
        -1
    )  # normalising to the maximum dose so that we can later directly apply the threshold
    ...
    output = F.pad(output.unsqueeze(1), (1, 1, 1, 1, 1, 1), mode="replicate").squeeze(
        1
    )  # double padding last two if larger kernel in dim 1 (5 instead of 3)
    # Unfolding to extract the neighbours in each dimension
    kernel_size = 3
    output_unfolded = (
        output.unfold(1, kernel_size, 1)
        .unfold(2, kernel_size, 1)
        .unfold(3, kernel_size, 1)
    )  # 5 for the first dim because it is in length of 1 mm
    ...
    output_unfolded = output_unfolded.flatten(start_dim=1, end_dim=3).flatten(
        start_dim=2, end_dim=4
    ...
    output_threshold = output_unfolded[
        :, voxels_threshold
    ]  # only keeping those above the threshold
    diff = torch.abs(
        output_threshold - target_threshold.unsqueeze(0)
    )  # difference between the output and target

    gamma = (
        torch.amin(diff, dim=0) / tolerance
    )  # minimum among all neighbours, as in the gamma index function

    gamma_index_batch = (
        torch.sum(sigmoid((1 - gamma), beta)) / num_voxels_above_threshold
    )
...
def pymed_gamma(
    ...
    for idx in range(batch_target.shape[0]):
        ...
        target = mean_output + target * std_output
        axes_reference = (
            mm_per_voxel[0] * np.arange(output.shape[2]),
            mm_per_voxel[1] * np.arange(output.shape[3]),
            mm_per_voxel[2] * np.arange(output.shape[4]),
        )
        if random_subset is None:
            gamma = pymedphys.gamma(
                axes_reference,
                target.squeeze(0).squeeze(0).cpu().detach().numpy(),
                axes_reference,
                output.squeeze(0).squeeze(0).cpu().detach().numpy(),
...
def torch_cubic_interp1d_2d(x, y, x_new):
    ...
    d2ydx2_first = (
        2 * (alpha[0, None] * slopes[0, :] - dydx[0, :]) / ((1 - alpha[0]) * xdiff[0])
    )
    d2ydx2_last = (
        2
        * (dydx[-1, :] - beta[-1, None] * slopes[-1, :])
        / (xdiff[-1] * (1 - beta[-1]))
    )
...
def plot_slices(
    ...
    num_slice=None,
    plane="z",
    save_plot_dir="images/sample.jpg",
    num_steps=1,
    original_dose=None,
    ...
    input, output, target, beam_energy = get_input_output_target(
        trained_model, loader, device, num_steps=num_steps
    )
    n_plots = 3
    ...
    if CT_flag:  # This is if the network was trained with the CT as a second channel
        ...
        CT_manual is not None
    ):  # Alternatively, the user can manually pass the CT as input
    ...
    column_titles = [
        ...
        "Actual \n Delivered Dose",
        "Abs. Error \n |Actual - Estimated|",
        f"Gamma Pass ({tolerance}%, {distance_mm_threshold}mm) \n Actual vs Estimated",
    ]
    ...
    for idx in range(n_plots):
        ...
        if num_slice is None:
            ...
            idcs_max_target = [
                target_img.shape[0] // 2,
                target_img.shape[1] // 2,
                target_img.shape[2] // 2,
            ]
        else:
        ...
        if plane == "z":
            ...
            axes_reference = (
                mm_per_voxel[2] * np.arange(output.shape[4]),
                mm_per_voxel[1] * np.arange(output.shape[3]),
            )  # Inverted because of the way we are plotting later
        ...
        if CT_flag:
            ...
            mask_input = (
                (input_img - torch.min(input_img))
                / (torch.max(input_img) - torch.min(input_img))
            ).numpy() * 1.4
            mask_out = (
                (out_img - torch.min(out_img))
                / (torch.max(out_img) - torch.min(out_img))
            ).numpy() * 1.4
            mask_target = (
                (target_img - torch.min(target_img))
                / (torch.max(target_img) - torch.min(target_img))
            ).numpy() * 1.4
            ...
            if plane == "z":
                ...
                CT_idx = np.array(
                    CT[idx].cpu().detach().squeeze(0).squeeze(0)[:, num_slice, :]
                )
            ...
            mask_input = np.ones_like(input_img).astype(
                float
            )  # Leave all if no CT is provided
            mask_target = np.ones_like(input_img).astype(
                float
            )  # Leave all if no CT is provided
            mask_out = np.ones_like(mask_out).astype(
                float
            )  # Leave all if no CT is provided
            mask_diff = np.ones_like(mask_diff).astype(
                float
            )  # Leave all if no CT is provided
        ...
        axs[idx, 1].imshow(
            np.flipud(out_img).T, cmap="jet", aspect="auto", alpha=mask_out, norm=norm
        )
        axs[idx, 1].set_xticks([])
        axs[idx, 1].set_yticks([])
        c2 = axs[idx, 2].imshow(
            np.flipud(target_img).T,
            cmap="jet",
            aspect="auto",
            alpha=mask_target,
            norm=norm,
        ...
        axs[idx, 3].imshow(
            np.flipud(diff_img).T, cmap="jet", aspect="auto", alpha=mask_diff, norm=norm
        )
        ...
        c3 = axs[idx, 4].imshow(
            np.flipud(gamma_img).T, cmap=cmap, aspect="auto", alpha=mask_diff, norm=norm
        )  ### vmax=1.0)
        axs[idx, 4].set_xticks([])
        axs[idx, 4].set_yticks([])

    if not isinstance(beam_energy[0], str) and not isinstance(
        beam_energy[0], torch.Tensor
    ):
    ...
    fig.text(
        0.0, 0.77, text_1, va="center", rotation="vertical", fontsize=font_size
    )  # , fontstyle='italic')
    fig.text(
        0.0, 0.48, text_2, va="center", rotation="vertical", fontsize=font_size
    )  # , fontstyle='italic')
    fig.text(
        0.0, 0.21, text_3, va="center", rotation="vertical", fontsize=font_size
    )  # , fontstyle='italic')

    cbar_ax1 = fig.add_axes([0.03, 0.01, 1 / num_cols * 0.85, 0.03])
    cbar_ax2 = fig.add_axes([1 / num_cols * 1.15, 0.01, 0.55, 0.03])
    gamma_cbar_ax = fig.add_axes(
        [1 / num_cols * (num_cols - 0.94), 0.01, 1 / num_cols * 0.85, 0.03]
    )
...
def plot_sample(
    ...
    input, estimated_dose, actual_dose, beam_energy = get_input_output_target(
        trained_model, loader, device, num_steps=num_steps
    )
    ...
    estimated_dose_scaled = (
        mean_dose + estimated_dose * std_dose
    ...
    if max_dose is not None:
        estimated_dose_scaled = (
            estimated_dose_scaled / torch.max(estimated_dose_scaled) * max_dose
        )
        actual_dose_scaled = (
            actual_dose_scaled / torch.max(actual_dose_scaled) * max_dose
        )
    ...
    column_titles = [
        "(a) Planned Dose",
        "(b) Actual\n Delivered Dose",
        f"(c) Gamma Test ({tolerance}%, {distance_mm_threshold}mm) \n Planned vs Actual",
        "(d) Input:\nPET",
        "(e) Output:\nEstimated Delivered Dose",
        f"(f) Gamma Test ({tolerance}%, {distance_mm_threshold}mm) \n Actual vs Estimated",
    ]
    ...
    if num_slice is None:
        # idcs_max_target = torch.where(target_img == torch.max(target_img)).item()
        idcs_max_actual_dose = [
            actual_dose_img.shape[0] // 2,
            actual_dose_img.shape[1] // 2,
            actual_dose_img.shape[2] // 2,
        ]
    else:
    ...
    if plane == "z":
        axes_reference = (
            mm_per_voxel[0] * np.arange(estimated_dose.shape[2]),
            mm_per_voxel[1] * np.arange(estimated_dose.shape[3]),
        )
        ...
        axes_reference = (
            mm_per_voxel[0] * np.arange(estimated_dose.shape[2]),
            mm_per_voxel[2] * np.arange(estimated_dose.shape[4]),
        )
    ...
    gamma_img = pymedphys.gamma(
        axes_reference,
        planned_dose_img.numpy(),
        axes_reference,
        actual_dose_img.numpy(),
        dose_percent_threshold=tolerance,
        distance_mm_threshold=distance_mm_threshold,
        lower_percent_dose_cutoff=threshold,
    )
    ...
    if CT_flag:
        ...
        mask_PET = (
            (PET_img - torch.min(PET_img)) / (torch.max(PET_img) - torch.min(PET_img))
        ).numpy() * 1.4
        mask_estimated_dose = (
            (estimated_dose_img - torch.min(estimated_dose_img))
            / (torch.max(estimated_dose_img) - torch.min(estimated_dose_img))
        ).numpy() * 1.4
        mask_planned_dose = (
            (planned_dose_img - torch.min(planned_dose_img))
            / (torch.max(planned_dose_img) - torch.min(planned_dose_img))
        ).numpy() * 1.4
        mask_actual_dose = (
            (actual_dose_img - torch.min(actual_dose_img))
            / (torch.max(actual_dose_img) - torch.min(actual_dose_img))
        ).numpy() * 1.4
        # mask_diff = ((diff_img - torch.min(diff_img)) / (torch.max(diff_img) - torch.min(diff_img))).numpy() * 1.4
        ...
        if plane == "z":
            ...
            CT_idx = np.array(
                CT[0].cpu().detach().squeeze(0).squeeze(0)[:, num_slice, :]
            )
            CT_idx = np.flip(CT_idx, axis=1)
        elif plane == "x":
            CT_idx = np.array(
        ...
        for plot_column in range(num_cols):
            axs[plot_column].imshow(
                np.flipud(CT_idx).T, cmap="gray", vmin=vmin_CT, vmax=vmax_CT, alpha=0.99
            )  # vmin and vmax modified to see PET

    else:
        mask_PET = np.ones_like(PET_img).astype(float)  # Leave all if no CT is provided
        mask_actual_dose = np.ones_like(PET_img).astype(
            float
        )  # Leave all if no CT is provided
        mask_estimated_dose = np.ones_like(mask_estimated_dose).astype(
            float
        )  # Leave all if no CT is provided
        mask_planned_dose = np.ones_like(mask_planned_dose).astype(
            float
        )  # Leave all if no CT is provided
        mask_diff = np.ones_like(mask_diff).astype(
            float
    ...
    axs[0].imshow(
        np.flipud(planned_dose_img).T,
        cmap="jet",
        aspect="auto",
        alpha=mask_planned_dose,
        norm=norm,
    )
    axs[0].set_xticks([])
    axs[0].set_yticks([])

    c_actual = axs[1].imshow(
        np.flipud(actual_dose_img).T,
        cmap="jet",
        aspect="auto",
        alpha=mask_actual_dose,
        norm=norm,
    )
    axs[1].set_xticks([])
    axs[1].set_yticks([])
    cbar_ax_actual = fig.add_axes([0.01, 0.0, 0.32, 0.07])
    cbar_actual = fig.colorbar(
        c_actual, cax=cbar_ax_actual, orientation="horizontal"
    )  # , format='%.1e')
    ...
    cmap_gamma = mcolors.ListedColormap(
        ["red", "green"]
    )  # if gamma is below 1.0, it is green, otherwise red
    c_planned_actual = axs[2].imshow(
        np.flipud(gamma_img_planned_actual).T,
        cmap=cmap_gamma,
        aspect="auto",
        alpha=mask_diff,
        norm=norm,
    ...
    gamma_cbar_planned_actual = fig.colorbar(
        c_planned_actual, cax=gamma_cbar_ax_planned_actual, orientation="horizontal"
    )
    gamma_cbar_planned_actual.set_label(label=r"Fail      Pass", size=font_size)
    gamma_cbar_planned_actual.ax.tick_params(
        labelsize=0, grid_alpha=0, length=0
    )  # Remove the tick marks
    ...
    c_estimated = axs[4].imshow(
        np.flipud(estimated_dose_img).T,
        cmap="jet",
        aspect="auto",
        alpha=mask_estimated_dose,
        norm=norm,
    )
    axs[4].set_xticks([])
    axs[4].set_yticks([])
    cbar_ax_estimated = fig.add_axes([0.673, 0.0, 0.15, 0.07])
    cbar_estimated = fig.colorbar(
        c_estimated, cax=cbar_ax_estimated, orientation="horizontal"
    )  # , format='%.1e')
    ...
    c_actual_estimated = axs[5].imshow(
        np.flipud(gamma_img_actual_estimated).T,
        cmap=cmap_gamma,
        aspect="auto",
        alpha=mask_diff,
        norm=norm,
    ...
    gamma_cbar_actual_estimated = fig.colorbar(
        c_actual_estimated, cax=gamma_cbar_ax_actual_estimated, orientation="horizontal"
    )
    gamma_cbar_actual_estimated.set_label(label=r"Fail      Pass", size=font_size)
    gamma_cbar_actual_estimated.ax.tick_params(
        labelsize=0, grid_alpha=0, length=0
    )  # Remove the tick marks
...
def plot_ddp(
    ...
    mean_output=0,
    std_output=1,
    save_plot_dir="images/ddp.jpg",
    num_steps=1,
    original_dose=None,
):
    input, output, target, _ = get_input_output_target(
        trained_model, loader, device, num_steps=num_steps
    )
    sns.set()
    n_plots = 3
    ...
    for idx in range(n_plots):
        ...
        z_slice_idx = idcs_max_target[
            -1
        ].item()  # Plotting the slice where the value of the dose is maximum
        y_slice_idx = idcs_max_target[
            -2
        ].item()  # Plotting the slice where the value of the dose is maximum
        ...
        axs[idx].plot(
            distance, target_profile, label="Target Deviated Dose", linewidth=2
        )
        if original_dose is not None:
            axs[idx].plot(
                distance,
                original_profile,
                label="Reference Treatment Dose",
                linewidth=2,
            )
...
def plot_adp(
    ...
    for idx in range(n_plots):
        ...
        distance = np.flip(np.arange(len(out_profile)))
        axs[idx].plot(
            distance, in_profile, label="Low-Statistics (scaled)", linewidth=2
        )
        axs[idx].plot(
            distance, out_profile, label="AI-generated High-Statistics", linewidth=2
        )
        axs[idx].plot(
            distance, target_profile, label="Simulated High-Statistics", linewidth=2
        )
...
def get_input_output_target(trained_model, loader, device, num_steps=1):
    ...
    input, output, target, third_output = (
        torch.tensor([]),
        torch.tensor([]),
        torch.tensor([]),
        torch.tensor([]),
    ...
    while input.shape[0] < 3:
        ...
        input_i, target_i, third_output_i = next(
            iter_loader
        )  # third output is either beam energy, beam number or time step, depending on the dataset

        if isinstance(input_i, list):
            output_i = 0
            for seed in range(len(input_i)):
                if num_steps == 1:
                    output_i += trained_model(
                        input_i[seed].to(device)
                    ).detach().cpu() / len(input_i)
                else:
                    t = third_output_i[0].to(device)
                    output_i_t = input_i[seed].clone().to(device)
                    for i in range(num_steps):
                        output_i_t = (
                            output_i_t - trained_model(output_i_t, t).detach().cpu()
                        )
...
def apply_model_to_patches(trained_model, input, target, patch_size):
    ...
    if x_patch_start[-1] != x_stop - 1:
        x_patch_start.append(x_stop - 1)
    y_stop = input.shape[2] - patch_size + 1
    y_patch_start = list(range(0, y_stop, patch_size))
    if y_patch_start[-1] != y_stop - 1:
        y_patch_start.append(y_stop - 1)
    z_stop = input.shape[3] - patch_size + 1
    z_patch_start = list(range(0, z_stop, patch_size))
    if z_patch_start[-1] != z_stop - 1:
        z_patch_start.append(z_stop - 1)

    for x in x_patch_start:
        for y in y_patch_start:
            for z in z_patch_start:
                input_patches.append(
                    input[:, x : x + patch_size, y : y + patch_size, z : z + patch_size]
                )
                target_patches.append(
                    target[
                        :, x : x + patch_size, y : y + patch_size, z : z + patch_size
                    ]
                )
                positions.append((x, y, z))

    # Apply model to each patch
    for input_patch, target_patch, (x, y, z) in zip(
        input_patches, target_patches, positions
    ):
...
def sobp_comparison(
    ...
    output_transform=None,
    SOBP_dataset=False,
    weights_dir="../activity-super-resolution/data/numbers-sobp.dat",
    save_slices_dir="images/sobp-slices.jpg",
    ...
    with torch.no_grad():
        if SOBP_dataset:
            for seed_loader in loaders:
                for batch_input, batch_target, _, _, _ in tqdm(seed_loader):
                    ...
                    if output_transform is None:
                        output_transform = CustomNormalize(
                            mean=mean_output.to(device), std=std_output.to(device)
                        )
            ...
            if isinstance(loaders, DataLoader):
                loaders = [
                    loaders
                ]  # Put into a list to be able to handle arbitraril many datasets corresponding to the same beams so that we can average them correspondingly
            for loader in loaders:
                for (
                    ...
                    for i in range(batch_input.shape[0]):
                        weight_beam = weights_dict[
                            float(beam_numbers[i])
                        ]  # Building the SOBP from the weights
                        input_img = batch_input[i].unsqueeze(0)
                        output_img = batch_output[i].unsqueeze(0)
                        target_img = batch_target[i].unsqueeze(0)
                        sobp_input += (
                            input_img * weight_beam / len(loaders)
                        )  # averaging over the three sets
        ...
        pymed_gamma_index, gamma_value = pymed_gamma(
            sobp_output,
            sobp_target,
            dose_percent_threshold=tolerance * 100,
            distance_mm_threshold=distance_mm_threshold,
            threshold=threshold,
        )

        l2_loss_input_val = l2_loss(sobp_input, sobp_target)
        pymed_gamma_index_input, gamma_value_input = pymed_gamma(
            sobp_input,
            sobp_target,
            dose_percent_threshold=tolerance * 100,
            distance_mm_threshold=distance_mm_threshold,
        ...
        text_results = (
            f"Difference between simulated high count and simulated low count SOBPs: \n"
    ...
    if (
        ...
        for plot_column in range(4):
            axs[plot_column].imshow(
                np.flipud(CT_idx).T, cmap="gray", vmin=vmin_CT, vmax=vmax_CT
            )
...
def get_CT(
    filepath="../deep-learning-dose-activity-dictionary/data/CT.npy",
    CT_statistics=(65.3300, 170.0528),
    img_size=(160, 64, 64),
    Trans=(-20, 0, -10),
    CT_flag=False,
):
    ...
    cropped_CT = large_CT[
        large_CT.shape[0] // 2 + TransX - HLX : large_CT.shape[0] // 2 + TransX + HLX,
    ...
    CT = zoom(
        cropped_CT,
        (
            img_size[0] / cropped_CT.shape[0],
            img_size[1] / cropped_CT.shape[1],
            img_size[2] / cropped_CT.shape[2],
        ),
    )
...
def input_vs_reference(
    input_dir,
    reference_dir,
    pymed_gamma=False,
    mm_per_voxel=(1.9531, 1.9531, 1.9531),
    num_samples=5,
    comparison_dir="data/comparison.csv",
):
    ...
    for sample in os.listdir(input_dir)[:num_samples]:
        input_sample = (
            torch.tensor(np.load(os.path.join(input_dir, sample)), dtype=torch.float32)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        reference_sample = (
            torch.tensor(
                np.load(os.path.join(reference_dir, sample)), dtype=torch.float32
            )
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        if pymed_gamma:
            gamma_pymed, gamma_value = pymed_gamma(
                input_sample,
                reference_sample,
    ...
    text_results = (
        f"L2 Loss: {torch.mean(l2_loss_list_torch)} +- {torch.std(l2_loss_list_torch)}\n"
    ...
    data = [
        input_dir,
        reference_dir,
        torch.mean(l2_loss_list_torch).item(),
        torch.std(l2_loss_list_torch).item(),
        torch.mean(psnr_list_torch).item(),
        torch.std(psnr_list_torch).item(),
        torch.mean(gamma_pymed_list_torch).item(),
        torch.std(gamma_pymed_list_torch).item(),
        torch.mean(gamma_value_pymed_list_torch).item(),
        torch.std(gamma_value_pymed_list_torch).item(),
    ]
...
def gaussian_input_vs_reference(
    input_dir,
    reference_dir,
    pymed_gamma=False,
    num_samples=5,
    ...
    for i in range(n_steps + 1):
        ...
        for sample in os.listdir(input_dir)[:num_samples]:
            ...
            reference_sample = (
                reference_sample - torch.mean(reference_sample)
            ) / torch.std(reference_sample)
...
def compare_plan_pet(
    ...
    for i in range(num_samples):
        c0 = axs[i].imshow(
            np.flipud(sample_slices[i]).T, cmap="inferno_r", norm=norm_act
        )
