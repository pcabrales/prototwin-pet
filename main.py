...
if deviations:
    ...
    patience = (
        20  # start with a higher patience for deviations, since training is less stable
    )
else:
...
mean_output, std_output = dataset_statistics(
    output_dir, scaling, num_samples=num_samples
)
dataset_statistics_dict = {
    input_dir: {"mean_input": mean_input.item(), "std_input": std_input.item()},
    output_dir: {"mean_output": mean_output.item(), "std_output": std_output.item()},
}
...
in_channels = 1
CT = np.load(
    os.path.join(script_dir, f"data/{patient_name}/dataset{dataset_num}/CT.npy")
)
...
dataset = SOBPDoseActivityDataset(
    input_dir=input_dir,
...
for idx, test_sample in enumerate(test_dataset):
    deviations_idx = test_sample[
        2
    ]  # dataset returns a tuple (image, target, deviations)
