...
num_rings = 200 # 322, reduced to fit in GPU
...
scanner = parallelproj.RegularPolygonPETScannerGeometry(
    ...
    radius=radius,
    num_sides=38,
    num_lor_endpoints_per_side=12, ###20, reduced to reduce memory usage
    lor_spacing=3.84,  # 3.2, lor_spacing to match num_lor_endpoints_per_side
...
dtype = xp.float32
dataset_folder = os.path.join("/home/pablo/prototwin/deep-learning-dose-activity-dictionary/data/sobp-dataset6")
...
for n_sobp, sobp in enumerate(os.listdir(activation_uncropped_npy_location)):
    ...
    activity = np.load(os.path.join(activation_uncropped_npy_location, sobp))
    ...
    if n_sobp % 10 == 0:
        ...
        plt.savefig(os.path.join(dataset_folder, f"{sobp[:-4]}.png"))
