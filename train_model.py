...
def train(
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, T_max=epochs, eta_min=2e-6
    )
    ...
    for epoch in range(epochs):
        ...
        for batch_input, batch_target, deviations_target in tqdm(train_loader):
            ...
            if deviations:
                batch_output, deviations_output = model(
                    batch_input, return_bottleneck=True
                )
                loss += alpha_deviation * l2_loss(
                    deviations_output, deviations_target.to(device)
                )
            else:
        ...
        with torch.no_grad():
            for batch_input, batch_target, deviations_target in tqdm(val_loader):
                ...
                if deviations:
                    ...
                    print(
                        deviations_output[0].detach().cpu().numpy(),
                        deviations_target[0].detach().cpu().numpy(),
                        loss.item(),
                    )
                else:
                    batch_output = model(batch_input)  # generating images
